...
├── esp32_client_last/      # ESP32 메인 펌웨어 (PlatformIO)
│   ├── platformio.ini
│   └── src/main.cpp
├── esp32/
│   ├── esp32_client_last0225/   # ESP32 + TM1637 변형 (PlatformIO)
│   │   ├── platformio.ini
│   │   └── src/main.cpp
│   └── esp_cam_entrance_v1_2_0225/  # ESP32-CAM 보안카메라 (PlatformIO)
│       ├── platformio.ini
│       └── src/main.cpp
├── esp_bluetooth_test/     # ESP32 블루투스 MP3 플레이어 (PlatformIO)
│   ├── platformio.ini
│   └── src/main.cpp
├── protocol/
...
├── models/                 # 대용량 모델 (MODELS.md 참조)
├── tests/                  # 테스트
├── docs/                   # 개발 문서
├── .env_example            # 민감 정보 템플릿
...
`.env` 파일이 없으면 서버 시작 시 터미널에서 직접 입력합니다.

| 변수 | 설명 |
|------|------|
| `PICOVOICE_ACCESS_KEY` | Picovoice 웨이크워드 API 키 |
| `MIC_DEVICE` | 마이크 디바이스 번호 (`arecord -l`로 확인) |
| `DB_USER` | MySQL 사용자명 |
...
```bash
# init_db.sql 내 사용자명/비밀번호를 수정 후 실행
...
### 8. ESP32 펌웨어 빌드 & 업로드 (PlatformIO)

```bash
# 메인 홈 컨트롤러
cd esp32_client_last && pio run -t upload

# ESP32-CAM 보안카메라
cd esp32/esp_cam_entrance_v1_2_0225 && pio run -t upload

# 블루투스 MP3 플레이어
cd esp_bluetooth_test && pio run -t upload
```

> 업로드 전 `src/main.cpp`에서 WiFi SSID/PW, 서버 IP를 환경에 맞게 수정하세요.
