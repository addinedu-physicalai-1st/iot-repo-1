...
├── esp32/                  # ESP32 펌웨어 소스
│   ├── config.h.example    # ESP32 WiFi/IP 설정 템플릿 (→ config.h로 복사)
├── gui/
...
민감정보(API 키, DB 비밀번호 등)는 `.env` 파일로 관리합니다.

```bash
# .env_example을 복사하여 .env 생성
cp .env_example .env
...
| 변수 | 설명 | 예시 |
|------|------|------|
| `MIC_DEVICE` | 마이크 디바이스 인덱스 (`python -m sounddevice`로 확인) | `11` |
...
ESP32의 WiFi 및 서버 IP 설정은 `esp32/config.h` 파일로 관리합니다.

```bash
# config.h.example을 복사하여 config.h 생성
cd esp32
...
> `esp32/config.h`는 `.gitignore`에 포함되어 있어 Git에 커밋되지 않습니다.
