 ...
 */
 ...
 // 서버 설정
 // ──────────────────────────────────────────
 const char*    SERVER_IP   = "192.168.0.189";  // Python 서버 IP
...
// ──────────────────────────────────────────
// 파라미터 (FPS↑·지연↓: FRAME_INTERVAL_MS 줄이기)
// ──────────────────────────────────────────
#define FRAME_INTERVAL_MS  100    // 캡처 간격 (~10fps), 50→~20fps
#define UDP_MTU            1400
#define FLASH_LED_PIN        4
#define JPEG_QUALITY_MIN     8    // 적응형 하향 시 최소 품질
#define JPEG_QUALITY_MAX    25    // 기본·복구 시 최대 품질
...
uint32_t frameId    = 0;
bool     camReady   = false;
sensor_t* camSensor = nullptr;     // 런타임 품질 조정용
int      jpegQuality = 15;        // 현재 JPEG 품질 (quality_down/up 적용)
 ...
 bool initCamera() {
  ...
  if (hasPsram) {
    ...
    Serial.println("[CAM] 모드: PSRAM → QVGA(320x240) q=15");
  } else {
    jpegQuality = 12;
    config.frame_size   = FRAMESIZE_QVGA;
    config.jpeg_quality = jpegQuality;
 ...
 void checkServerCommand() {
  ...
  } else if (cmd.indexOf("quality_down") >= 0 && camSensor) {
    jpegQuality = max(JPEG_QUALITY_MIN, jpegQuality - 3);
    camSensor->set_quality(camSensor, jpegQuality);
    Serial.printf("[CAM] 품질 하향 → q=%d\n", jpegQuality);
  } else if (cmd.indexOf("quality_up") >= 0 && camSensor) {
    jpegQuality = min(JPEG_QUALITY_MAX, jpegQuality + 3);
    camSensor->set_quality(camSensor, jpegQuality);
    Serial.printf("[CAM] 품질 상향 → q=%d\n", jpegQuality);
  }
}
