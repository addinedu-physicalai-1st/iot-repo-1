...
[ESP32]          [TM1637 모듈]
 GPIO22 ─────── CLK  (황)
 GPIO23 ─────── DIO  (백)
 5V    ─────── VCC  (적) ← 반드시 5V (3.3V 불가, 미표시 또는 어두움)
 GND   ─────── GND  (흑)
```

> caps 등록값: `["led", "seg7"]`
>
> TM1637은 5V 모듈이지만 CLK/DIO 신호는 ESP32 3.3V GPIO로 정상 통신됨.
> VCC만 5V 공급, 신호선은 GPIO 직결 가능.
...
[TCP] 연결 성공
[TCP] 등록: {"type":"register","device_id":"esp32_bathroom","caps":["led","seg7"]}
[SEG7] 초기화 완료
```
...
[TCP] 연결 성공
[TCP] 등록: {"type":"register","device_id":"esp32_bedroom","caps":["led","servo"]}
```
...
□ esp32_bathroom 업로드
  ...
  □ 업로드 완료
  □ 시리얼: caps=["led","seg7"] 확인
  □ 7세그먼트 표시 확인  (TM1637 VCC = 5V 확인)
  □ 대시보드 욕실 카드 → ONLINE

□ esp32_bedroom 업로드
  □ DEVICE_ID = "esp32_bedroom" 설정
  □ 업로드 완료
  □ 시리얼: caps=["led","servo"] 확인
  □ 서보(커튼) 동작 확인
  □ 대시보드 침실 카드 → ONLINE
...
*Voice IoT Controller · ESP32_UPLOAD_GUIDE · 2026-02-22 (v3 — TM1637 모듈 확정, 최종 구성)*
